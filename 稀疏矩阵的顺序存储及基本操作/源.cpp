...
Status TransposeMatrix1(RLSMatrix M, RLSMatrix& N) {//转置矩阵方法1
	...
	int q = 1;
	for (int a = 1;a < M.nu + 1;a++) {//在M中找N的每一个元素，而M按行序存储，所以只要在M中找到对应行的列就是N的行；
...
Status AddMatrix(RLSMatrix M, RLSMatrix N, RLSMatrix& Q) {//矩阵加法
	Q.mu = M.mu;Q.nu = M.nu;Q.tu = 1;
	int temp[MAXSIZE];
	int t;
	for (int col = 1;col < Q.mu + 1;col++) {
		for (int j = 0;j < MAXSIZE;j++) temp[j] = 0;
		for (int row = 1;row < Q.nu + 1;row++) {
			if (t = Check(M, col, row)) {
				if (t = Check(N, col, row)) {
					temp[row] = M.data[t].v + N.data[t].v;
				}
				else temp[row] = M.data[t].v;
			}
			else {
				if (t = Check(N, col, row)) {
					temp[row] = N.data[t].v;
		...
		for (int i = 1;i < Q.nu + 1;i++) {
			if (temp[i]) {
				Q.data[Q.tu].v = temp[i];
				Q.data[Q.tu].i = col;
				Q.data[Q.tu].j = i;
			}
			Q.tu++;
